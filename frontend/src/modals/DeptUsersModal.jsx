...
export default function DeptUsersModal({ isOpen, dept, onClose }) {
  const [page, setPage] = useState(1)
  const [search, setSearch] = useState('') // إضافة حالة البحث

  const { data, isLoading } = useQuery({
    // أضفنا search لـ queryKey ليعيد جلب البيانات عند البحث
    queryKey: ['dept-users', dept.id, page, search],
    queryFn: () => orgApi.usersDept(dept.id, { 
  ...
  return (
    <Modal open={isOpen} onClose={onClose} title={dept.name} size="md">

...
import { X, Save, Loader2, HelpCircle, Network } from 'lucide-react'
...
export default function RecurringTaskFormModal({ id, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl text-right flex flex-col my-8" dir="rtl">
        ...
        {isLoadingData ? (
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4 flex-1 overflow-y-auto max-h-[calc(100vh-200px)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">عنوان المهمة القالب *</label>
                <input
                  type="text"
                  name="title"
              ...
              <div>
                <div className="flex items-center gap-1 mb-1">
                  <label className="block text-xs font-bold text-gray-700">القسم المستهدف للمهمة *</label>
                  <button type="button" onClick={() => toggleHint('dept')} className="text-gray-400 hover:text-indigo-600 transition-colors">
                    <HelpCircle className="w-3.5 h-3.5" />
                  </button>
                </div>
                
                {activeHint.dept && (
                  <div className="mb-2 p-2.5 bg-indigo-50/70 border border-indigo-100 rounded-lg text-[11px] text-indigo-900 leading-relaxed transition-all animate-fadeIn">
                ...
                <div className="relative flex items-center">
                  <input
                    ...
                    className={`w-full border border-gray-200 rounded-lg p-2 text-xs text-right cursor-pointer pl-10 focus:outline-none ${
                      selectedDeptName ? 'text-indigo-900 font-bold bg-indigo-50/30' : 'text-gray-400 bg-gray-50'
                    }`}
                  />
                  <button
                    type="button"
                    onClick={() => setIsTreeOpen(true)}
                    className="absolute left-2 p-1 text-gray-400 hover:text-indigo-600 transition-colors"
                  >
            ...
            <div>
              <div className="flex items-center gap-1 mb-1">
                <label className="block text-xs font-bold text-gray-700">وصف المهمة وتعليمات التنفيذ</label>
                <button type="button" onClick={() => toggleHint('desc')} className="text-gray-400 hover:text-indigo-600 transition-colors">
                  <HelpCircle className="w-3.5 h-3.5" />
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">درجة الأولوية</label>
                <select
                  name="priority"
              ...
              <div>
                <div className="flex items-center gap-1 mb-1">
                  <label className="block text-xs font-bold text-gray-700">نمط التكرار الدوري *</label>
                  <button type="button" onClick={() => toggleHint('pattern')} className="text-gray-400 hover:text-indigo-600 transition-colors">
                    <HelpCircle className="w-3.5 h-3.5" />
                  </button>
                </div>
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full border border-gray-200 rounded-lg p-2 text-sm focus:outline-indigo-600 font-medium"
                >
              ...
              <div>
                <div className="flex items-center gap-1 mb-1">
                  <label className="block text-xs font-bold text-gray-700">معدل الفاصل الزمني (Interval)</label>
                  <button type="button" onClick={() => toggleHint('interval')} className="text-gray-400 hover:text-indigo-600 transition-colors">
                    <HelpCircle className="w-3.5 h-3.5" />
                  </button>
                </div>

                <div className="flex items-center gap-1.5">
                  <span className="text-xs text-gray-400">كل</span>
                  <input
                    type="number"
                    name="interval_value"
                    min="1"
                    ...
                    onChange={handleChange}
                    className="w-full border border-gray-200 rounded-lg p-2 text-sm text-center font-mono focus:outline-indigo-600"
                  />
                </div>
              </div>
            </div>
            ...
            {formData.recurrence_pattern === 'weekly' && (
              <div className="bg-blue-50/50 border border-blue-100 p-3 rounded-lg animate-fadeIn">
                <label className="block text-xs font-bold text-blue-900 mb-1">تحديد يوم الصدور الأسبوعي:</label>
                <select
                  name="day_of_week"
                  value={formData.day_of_week}
                  onChange={handleChange}
                  className="w-full border border-blue-200 rounded-lg p-2 text-sm bg-white focus:outline-indigo-600"
                >
                  <option value="">اختر يوم العمل الأسبوعي...</option>
            ...
            {formData.recurrence_pattern === 'monthly' && (
              <div className="bg-purple-50/50 border border-purple-100 p-3 rounded-lg animate-fadeIn">
                <label className="block text-xs font-bold text-purple-900 mb-1">تحديد تاريخ اليوم في الشهر (1 - 31):</label>
                <input
                  type="number"
                  name="day_of_month"
                  min="1"
                  ...
                  onChange={handleChange}
                  className="w-full border border-purple-200 rounded-lg p-2 text-sm bg-white font-mono focus:outline-indigo-600"
                  placeholder="مثال: 25"
                />
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">تاريخ التشغيل التلقائي القادم *</label>
                <input
                  type="date"
                  name="next_run_date"
                  value={formData.next_run_date}
                  onChange={handleChange}
                  className="w-full border border-gray-200 rounded-lg p-2 text-sm font-mono focus:outline-indigo-600"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">وقت صدور المهمة *</label>
                <input
                  type="time"
                  name="run_time"
            ...
            <div className="flex items-center justify-between bg-gray-50 p-3 rounded-lg border border-gray-100 mt-2">
              <div className="flex flex-col text-right">
                <span className="text-xs font-bold text-gray-700">حالة نشاط أتمتة القالب</span>
              </div>
